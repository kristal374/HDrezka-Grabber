...
async function startLoadVideo(tab_ID) {
    ...
    if (data[key].displaySettings.load_all_series) {
        ...
        for (let s of seasons.slice(
            seasons.indexOf(videoConfig.season_start)
        )) {
            let sliceIndex =
                s === videoConfig.season_start
                    ? episodes[s].indexOf(videoConfig.episode_start)
                    : 0;
...
async function initLoadVideo(tab_ID, settingsVideo) {
    ...
    if (settingsVideo.action === "get_movie") {
        ...
        filename =
            filename +
            "_S" +
            settingsVideo.season_id +
            "E" +
            settingsVideo.episode_id +
            ".mp4";
    }
    if (flagLoader) {

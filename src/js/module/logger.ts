...
browser.storage.local.get(["debugFlag"]).then((storage) => debugFlag = storage.debugFlag as boolean)
...
function checkingDebugFlag(_target: any, _key: string | symbol, descriptor: PropertyDescriptor): any {
...
export class Logger {
    ...
    private getCallerInfo() {
        ...
        const regexFindURL = /((?:chrome|moz)-extension:\/(?:\/.*?)+\.js:(\d+):(\d+))/g;
...
class SourceMapParser {
    ...
    public getOriginalPosition(line: number, column: number): {
        ...
        for (const segment of segments) {
            const [generatedColumn, sourceIndex, sourceLine, sourceColumn, nameIndex] = segment;
    ...
    public getOriginalURL(url: string): string {
        ...
        const originalSource = this.getOriginalPosition(parseInt(line), parseInt(column))
        if (!originalSource) return url
        return this.urlToFormat(originalSource!.source, originalSource!.line, originalSource!.column)
    }

    private urlToFormat(path: string, line?: number, column?: number) {
        return browser.runtime.getURL(path.replaceAll("../", "")) + `:${line}:${column}`
    }
